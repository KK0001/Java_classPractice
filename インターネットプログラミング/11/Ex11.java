...
class LineSeg {
    ...
    boolean isOnLineSeg(PointXY p) {
        ...
        if( (p.getX() >= small) && (big >= p.getX()) ){
          int a1 = a.getY() - b.getY();
          int b1 = b.getX() - a.getX();
          int c1 = (-b1 * a.getY()) + (-a1 * a.getX());
          return (a1 * p.getX() + b1 * p.getY() + c1) == 0;
        }else{
          return false;
        }
    }
...
class Ex11 {
    static String gakuban = "EC000"; // 学籍番号を入力すること
    static String yourname = "千";  // 氏名を入力すること
